...
const Settings = () => {
  ...
  useEffect(() => {
    const newPath = location.pathname.split('/settings/')[1] || '';
    setActiveSection(newPath || 'storage');
  }, [location.pathname]);

  // Handle section changes by updating URL and active section
  ...
  // Determine if a specific path is active
  const isActive = (path: string) => {
    return location.pathname === path;
  };

  // Storage section menu items
  const storageItems = [
    {
  ...
  // Notification section menu items
